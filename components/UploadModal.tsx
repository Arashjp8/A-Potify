    ...
    const onSubmit: SubmitHandler<FieldValues> = async (values) => {
        try {
            ...
            const {
                data: songData,
                error: songError
            } = await supabaseClient.storage.from("songs").upload(`song-${values.title}-${uniqeID}`, songFile, {
                cacheControl: "3600",
                upsert: false
            });
            ...
            const {
                data: imageData,
                error: imageError
            } = await supabaseClient.storage.from("images").upload(`image-${values.title}-${uniqeID}`, imageFile, {
                cacheControl: "3600",
                upsert: false
            });
